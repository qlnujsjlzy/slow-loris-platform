...
<#list attributeInfos as attributeInfo>
    ...
    <#else>
        @TableField(value = "${attributeInfo.dbName}"<#if !attributeInfo.isBaseType>,el="${attributeInfo.name}
        .id"</#if><#if attributeInfo.name??&&(attributeInfo.name='createBy'||attributeInfo.name='createDate')>,fill =
        FieldFill.INSERT</#if><#if attributeInfo.name??&&(attributeInfo.name='updateBy'||attributeInfo.name='updateDate')>
        ,fill = FieldFill.UPDATE</#if>)
    </#if>
...
<#list attributeInfos as attributeInfo>
    ...
    */
    public <#if attributeInfo.type=='this'>${entityName?cap_first}<#else>${attributeInfo.type}</#if>
    get${attributeInfo.name?cap_first}(){
    ...
    */
    public void set${attributeInfo.name?cap_first}
    (<#if attributeInfo.type=='this'>${entityName?cap_first}<#else>${attributeInfo.type}</#if> ${attributeInfo.name}){
