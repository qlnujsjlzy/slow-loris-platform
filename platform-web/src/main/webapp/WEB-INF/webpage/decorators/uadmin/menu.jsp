...
<li <c:if test="${currentMenu == null}">class="active"</c:if>><a href="${adminPath}"><i class="fa fa-tachometer fa-fw">
    <div class="icon-bg bg-orange"></div>
</i><span class="menu-title">控制台</span></a></li>
        ...
        <li <c:if test="${fn:contains(pmenuids,menu.id)==true||menu.id == currentMenu.id}">class="active"</c:if>>
            <c:choose>
                <c:when test="${menu.hasChildren}">
                    <a href="#"><i
                            class="fa <c:if test="${empty menu.menuIcon}">fa-desktop</c:if> ${menu.menuIcon} fa-fw">
                                    ...
                                    <c:choose>
                                        <c:when test="${secondMenu.hasChildren}">
                                            <a href="#"><i
                                                ...
                                                <span class="submenu-title">${secondMenu.name}</span><span
                                                ...
                                                <c:forEach items="${menus}" var="thirdMenu">
                                                    <c:if test="${thirdMenu.parentId == secondMenu.id && thirdMenu.isshow eq '1'}">
                                                        <li
                                                            ...
                                                            <a href="${adminPath}/${thirdMenu.url}"><i
                                                                    class="fa <c:if test="${empty thirdMenu.menuIcon}">fa-angle-double-right</c:if>${thirdMenu.menuIcon}"></i>
