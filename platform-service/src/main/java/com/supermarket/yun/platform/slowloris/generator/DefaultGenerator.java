...
public class DefaultGenerator implements IGenerator {
    ...
    protected String getRootPackage() {
        ...
        if (config.getLayer().contains("controller")){
            rootPackage = generatorProperties.getString("layer.controller");
        }else if(config.getLayer().contains("service")){
            rootPackage = generatorProperties.getString("layer.service");
        }else if(config.getLayer().contains("mapper")){
            rootPackage = generatorProperties.getString("layer.mapper");
        }else if(config.getLayer().contains("domain")){
            rootPackage = generatorProperties.getString("layer.domain");
        }
    ...
    protected String getOutPath(GeneratorInfo generatorInfo) {
        ...
        if (config.getCodeType() == CODE_TYPE_JSP) {
            outPath += File.separator + "WEB-INF/webpage/modules"+File.separator +moduleName;
        } else if (config.getCodeType() == CODE_TYPE_MAPPER) {
            outPath += File.separator + "/mappers"+File.separator +moduleName;
        } else {
        ...
        if (!"".endsWith(moduleName)) {
            outPath = outPath.replace("[module]", moduleName).toLowerCase();
        }
        //兼容配置文件目录
